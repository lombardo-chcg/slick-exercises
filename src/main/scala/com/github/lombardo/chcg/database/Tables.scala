...
trait Tables {
   ...
   *  @param bookid Database column bookid SqlType(int4), PrimaryKey
   *  @param facid Database column facid SqlType(int4)
   *  @param memid Database column memid SqlType(int4)
   *  @param starttime Database column starttime SqlType(timestamp)
   *  @param slots Database column slots SqlType(int4) */
  ...
  implicit def GetResultBookingsRow(implicit e0: GR[Int], e1: GR[java.sql.Timestamp]): GR[BookingsRow] = GR{
    prs => import prs._
  ...
  class Bookings(_tableTag: Tag) extends profile.api.Table[BookingsRow](_tableTag, Some("cd"), "bookings") {
    ...
    def ? = (Rep.Some(bookid), Rep.Some(facid), Rep.Some(memid), Rep.Some(starttime), Rep.Some(slots)).shaped.<>({r=>import r._; _1.map(_=> BookingsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val facilitiesFk = foreignKey("fk_bookings_facid", facid, Facilities)(r => r.facid, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
    /** Foreign key referencing Members (database name fk_bookings_memid) */
    lazy val membersFk = foreignKey("fk_bookings_memid", memid, Members)(r => r.memid, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
   ...
   *  @param facid Database column facid SqlType(int4), PrimaryKey
   *  @param name Database column name SqlType(varchar), Length(100,true)
   *  @param membercost Database column membercost SqlType(numeric)
   *  @param guestcost Database column guestcost SqlType(numeric)
   *  @param initialoutlay Database column initialoutlay SqlType(numeric)
   *  @param monthlymaintenance Database column monthlymaintenance SqlType(numeric) */
  case class FacilitiesRow(facid: Int, name: String, membercost: scala.math.BigDecimal, guestcost: scala.math.BigDecimal, initialoutlay: scala.math.BigDecimal, monthlymaintenance: scala.math.BigDecimal)
  /** GetResult implicit for fetching FacilitiesRow objects using plain SQL queries */
  implicit def GetResultFacilitiesRow(implicit e0: GR[Int], e1: GR[String], e2: GR[scala.math.BigDecimal]): GR[FacilitiesRow] = GR{
    prs => import prs._
    FacilitiesRow.tupled((<<[Int], <<[String], <<[scala.math.BigDecimal], <<[scala.math.BigDecimal], <<[scala.math.BigDecimal], <<[scala.math.BigDecimal]))
  }
  ...
  class Facilities(_tableTag: Tag) extends profile.api.Table[FacilitiesRow](_tableTag, Some("cd"), "facilities") {
    def * = (facid, name, membercost, guestcost, initialoutlay, monthlymaintenance) <> (FacilitiesRow.tupled, FacilitiesRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(facid), Rep.Some(name), Rep.Some(membercost), Rep.Some(guestcost), Rep.Some(initialoutlay), Rep.Some(monthlymaintenance)).shaped.<>({r=>import r._; _1.map(_=> FacilitiesRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
   ...
   *  @param memid Database column memid SqlType(int4), PrimaryKey
   *  @param surname Database column surname SqlType(varchar), Length(200,true)
   *  @param firstname Database column firstname SqlType(varchar), Length(200,true)
   *  @param address Database column address SqlType(varchar), Length(300,true)
   *  @param zipcode Database column zipcode SqlType(int4)
   *  @param telephone Database column telephone SqlType(varchar), Length(20,true)
   *  @param recommendedby Database column recommendedby SqlType(int4), Default(None)
   *  @param joindate Database column joindate SqlType(timestamp) */
  case class MembersRow(memid: Int, surname: String, firstname: String, address: String, zipcode: Int, telephone: String, recommendedby: Option[Int] = None, joindate: java.sql.Timestamp)
  /** GetResult implicit for fetching MembersRow objects using plain SQL queries */
  implicit def GetResultMembersRow(implicit e0: GR[Int], e1: GR[String], e2: GR[Option[Int]], e3: GR[java.sql.Timestamp]): GR[MembersRow] = GR{
    prs => import prs._
    MembersRow.tupled((<<[Int], <<[String], <<[String], <<[String], <<[Int], <<[String], <<?[Int], <<[java.sql.Timestamp]))
  }
  ...
  class Members(_tableTag: Tag) extends profile.api.Table[MembersRow](_tableTag, Some("cd"), "members") {
    def * = (memid, surname, firstname, address, zipcode, telephone, recommendedby, joindate) <> (MembersRow.tupled, MembersRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(memid), Rep.Some(surname), Rep.Some(firstname), Rep.Some(address), Rep.Some(zipcode), Rep.Some(telephone), recommendedby, Rep.Some(joindate)).shaped.<>({r=>import r._; _1.map(_=> MembersRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7, _8.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val membersFk = foreignKey("fk_members_recommendedby", recommendedby, Members)(r => Rep.Some(r.memid), onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.SetNull)
